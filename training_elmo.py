...
def get_batch(batch, old, new, max_len=0):
	"""
	...
	if max_len == 0:
		...
		max_len = max_len
	#print (tup[-1][-1], max_len)
	for embedded, entities, length in zip(tup[0], tup[2], tup[-1]):
		...
		e1, e2 = entities[0], entities[1]
		x = embedded[:len(length)]
		pos = list(range(1, len(x) + 1))
		...
		queries.append([embedded[e1], embedded[e2]])
		"""
	#print (np.shape(xs), np.shape(positions))
...
if __name__ == "__main__":
	...
	with tf.device(device):
		...
		flags.DEFINE_float("dropout", 0.10, "dropout applied after each layer")
		...
		flags.DEFINE_float("hidden_units_ffclayer", 4, "")
		flags.DEFINE_integer("num_epochs", 3, "")
		flags.DEFINE_string("activation", "relu", "")
		flags.DEFINE_string("use_positions", "yes", "")


		flags.DEFINE_string("labels_file","/raid/dost01/semeval10_data/labels.txt", "files with one label per line")
		...
		flags.DEFINE_string("sdp_file_train", "semeval_train.tsv", "filename of meta info train (positions queries, labels")
		flags.DEFINE_string("sdp_file_test", "semeval_test.tsv", "filename of meta info test (positions queries, labels")
		...
		flags.DEFINE_string("encoder_attention", "no", "")
		flags.DEFINE_string("encoder_feedforward", "no", "")
		flags.DEFINE_string("decoder_self_attention", "no", "")
		...
		flags.DEFINE_string("only_entities", "no", "")
		flags.DEFINE_string("optimizer", "default", "")
		flags.DEFINE_string("calculate_sdp_information", "no", "")
		...
		#FLAGS.max_steps = max(750,FLAGS.num_epochs * 8000 / FLAGS.batch_size + 1)
		FLAGS.warmup_steps = 0.1 * FLAGS.max_steps

		print (FLAGS.max_steps, FLAGS.warmup_steps)
		# initialize model
		transformers_model = model(preprocessing, FLAGS)
		init = tf.global_variables_initializer()
		#FLAGS.resultfile = "result_files/dropout_" + str(FLAGS.dropout) + "_batch_size_" + str(FLAGS.batch_size) + "_num_heads_" + str(FLAGS.num_heads)
		if FLAGS.resultfile == "default":
			FLAGS.resultfile = "result_files/dropout_" + str(FLAGS.dropout) + "_batch_size_" + str(FLAGS.batch_size) + "_l2_lambda_" + str(FLAGS.l2_lambda) + "_num_head" + str(FLAGS.num_heads)

		print (FLAGS.resultfile)
		f1_results = open(FLAGS.resultfile, "w")
		labels_dev = defaultdict(int)
		...
		with tf.Session(config=config) as sess:
			...
			print (att_mass_in, att_mass_out, att_mass_in/(att_mass_in+att_mass_out))
			print ("spd exists:", spd_exists, "out of 2712")
			print ("tokens in", tokens_in, "tokens_out", tokens_out, tokens_in / (tokens_in + tokens_out))
